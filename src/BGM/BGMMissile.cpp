...
// region Constructor
    ...
    Missile::Missile() {
        _fileSaver = std::make_shared<FileSaver>("./Results/BGM/");
        _maxLoad   = 30;
        _s         = 1.6;
        _l         = 6.55;
        _b         = 6.55;
...
// region Public Methods
    ...
    void Missile::setCruiseAltAndMH(const double &alt, const double &MH){
        _engine._MH=MH;
        _guidance.cruiseAltitude=alt;
    }
    ...
    double Missile::update() {
        ...
        if (dis<1000&&distance_deque.size() >= 4) {
            bool success = true;

...
// region Constructor
    ...
    Missile::Missile() {
        _fileSaver = std::make_shared<FileSaver>("./Results/LRHW/");
        _maxLoad   = 30;
        _s         = 3.2;
        _l         = 6.55;
        _b         = 6.55;
        ...
        _kinematics._dynamics._aerodynamics.computeAeroCoefficientsCB =
                [this](const double alpha, const double beta, const double dx, const double dy, const double dz, const double Ma, double &CD, double &CL, double &CZ, double &Cl,
                    ...
                    // 2. 计算马赫数相关的轴向力参数
                    const double CA0 = 0.02 * CN;
                    const double k   = 0.3 * CN;
                    // 3. 气动力系数
...
// region Public Methods
    ...
    void Missile::launch(const double theta_f_d, const double psi_f_d) {
        ...
        const Eigen::Vector3d vel_body = {300, 0, 0};
    ...
    double Missile::update() {
        ...
        if (_targetPosEcf.has_value()) {
            ...
            if(_routePoints.empty()) {
                ...
                auto targetPosEcf = Utils::CoordinateHelper::llaToEcef(_routePoints.front());
                auto targetVelEcf = Eigen::Vector3d{0,0,0};
                auto dis = (targetPosEcf-_state.posEcf).norm();
                const auto gcInfo = _guidance.getGCInfo(flyTime(), _p_body.norm(), _totalMass, targetPosEcf, targetVelEcf, _state, _maxLoad);
                losInfo = gcInfo.losInfo;
                acc_cmd_v = gcInfo.acc_cmd_v;
                if(dis<1000) {
                    _routePoints.pop_front();
                    std::cout<<flyTime()<<"--next"<<std::endl;
                }
            }
        ...
        if (dis<5000&&distance_deque.size() >= 4) {
            bool success = true;

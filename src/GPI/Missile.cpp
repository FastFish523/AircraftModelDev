...
// region Constructor
    ...
    Missile::Missile() {
        _fileSaver = std::make_shared<FileSaver>("./Results/GPI/");
        _maxLoad   = 30;
        _s         = 0.5;
        _l         = 6.55;
        _b         = 6.55;
...
// region Public Methods
    ...
    void Missile::launch(const double theta_f_d, const double psi_f_d, const double launch_v) {
        Eigen::Vector3d lla            = this->lla();
        _state.qbn                     = ModelDevelop::Utils::CoordinateHelper::euler231ToQuaternion(psi_f_d, theta_f_d, 0);
        _launch_v_body = launch_v;
        const Eigen::Vector3d vel_body = {launch_v, 0, 0};
    ...
    double Missile::update() {
        ...
        // 推力 质量 转动惯量更新
        const auto [mass, P_body, inertia] = _engine.getEigenInfo(_step, _flyTime,_state,_launch_v_body);
        _p_body                            = P_body;

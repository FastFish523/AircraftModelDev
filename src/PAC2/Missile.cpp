...
// region Constructor
    ...
    Missile::Missile() {
        _fileSaver = std::make_shared<FileSaver>("./Results/PAC2/");
        _maxLoad   = 30;
        _s         = 0.8;
        _l         = 6.55;
        _b         = 6.55;
        ...
        _kinematics._dynamics._aerodynamics.computeAeroCoefficientsCB =
                [this](const double alpha, const double beta, const double dx, const double dy, const double dz, const double Ma, double &CD, double &CL, double &CZ, double &Cl,
                    ...
                    CN =0.3+ 0.6*ma*ma / (1 + 0.8*ma*ma*ma*ma) + 4.0 / sqrt(1 + (ma*ma - 1)*(ma*ma - 1));
                    // 2. 计算马赫数相关的轴向力参数
...
// region Public Methods
    ...
    double Missile::update() {
        ...
        if (_targetPosEcf.has_value()) {
            const auto [losInfo, acc_cmd_v] = _guidance.getGCInfo(flyTime(), _p_body.norm(), _totalMass, _targetPosEcf.value(), _targetVelEcf, _state, _maxLoad);
            double alpha = this->alpha() / 57.3;
            // std::cout <<"1-- " <<alpha << std::endl;
            double beta  = this->beta() / 57.3;
            auto acc_cmd_b = ModelDevelop::Utils::CoordinateHelper::velocityToBodyAcceleration(acc_cmd_v, alpha, beta);
        ...
        if (targetDis()<100&&distance_deque.size() >= 4) {
            bool success = true;
    ...
    Eigen::Vector3d Missile::accelerationBody() const {
        return _imu_info.imu_acc_body;
    }

    Eigen::Vector3d Missile::w_xyz() const {
        return _imu_info.imu_w_xyz_body;
    }

    double Missile::mass() const {
        return _totalMass;
    }

    double Missile::P() const {
        return _p_body.norm();
    }

    Eigen::Vector3d Missile::targetPositionLaunchNUE() const {
        if (_targetPosEcf.has_value()) {
    ...
    Eigen::Vector3d Missile::rudder() const {
        return _rudder * 57.3;
    }

    Eigen::Vector3d Missile::velocityNUE() const {
        Eigen::Vector3d lla = ModelDevelop::Utils::CoordinateHelper::ecefToLla(_state.posEcf);
        return ModelDevelop::Utils::CoordinateHelper::ecefToNueVelocity(_state.velEcf, lla.x(), lla.y());
    }

    Eigen::Vector3d Missile::lla() const {
        Eigen::Vector3d lla = ModelDevelop::Utils::CoordinateHelper::ecefToLla(_state.posEcf);
        return lla;
    }

    double Missile::targetDis() const {
        if (_targetPosEcf.has_value())
            return (_state.posEcf - _targetPosEcf.value()).norm();
        return -1;
    }

    auto Missile::acc_cmd_b_y() const -> double {
        return _acc_cmd_b_y;
    }

    auto Missile::acc_cmd_b_z() const -> double {
        return _acc_cmd_b_z;
    }

    auto Missile::sigmaElv() const -> double {
        return _sigma_elv * 57.3;
    }

    auto Missile::sigmaElvDot() const -> double {
        return _sigma_elv_dot * 57.3;
    }

    auto Missile::sigmaAz() const -> double {
        return _sigma_az * 57.3;
    }

    auto Missile::sigmaAzDot() const -> double {

...
// region Constructor
    ...
    Missile::Missile() {
        _fileSaver = std::make_shared<FileSaver>("./Results/Su27/");
        _maxLoad   = 30;
        _s         = 45.000;
        _l         = 10;
        _b         = 10;
...
// region Public Methods
    ...
    void Missile::take_off(const double theta_f_d, const double psi_f_d) {
        Eigen::Vector3d lla            = this->lla();
        _state.qbn                     = ModelDevelop::Utils::CoordinateHelper::euler231ToQuaternion(psi_f_d, 1, 0);
    ...
    double Missile::update() {
        ...
        if (_flightMode == 0) {
            ...
            auto selfVel_nue                   = Utils::CoordinateHelper::ecefToNueVelocity(_state.velEcf,lla().x(),lla().y());
            const auto theta                   = Utils::CoordinateHelper::getTheta(selfVel_nue);

            const double Vy = selfVel_nue.y();
            double acc_cmd_v_y = 1.5*9.8*std::copysign(1.0, cos(_imu_info.imu_ypr.z()));//(0.05*(20000-lla().z())-2*4*0.05*Vy)*std::copysign(1.0, cos(_imu_info.imu_ypr.z()));
            acc_cmd_v_y = (0.05*(20000-lla().z())-2*4*0.05*Vy);
            Eigen::Vector3d acc_cmd_v={0,acc_cmd_v_y,0};
            ...
            static bool x=false;
            if(flyTime()-30.0 < 1e-100&&x==false) {
                yaw_C = Utils::CoordinateHelper::quaternionToEuler231(_state.qbn).x()+90;
                std::cout<<yaw_C.value()<<std::endl;

                x =true;
            }
            double rollC = 0;
            if(yaw_C.has_value()) {
                double e = (yaw_C.value() - Utils::CoordinateHelper::quaternionToEuler231(_state.qbn).x());
                rollC = 2*(-60./(1+exp(-1*e))+30)/57.3;
                // if(e<1/57.3) {
                //     yaw_C = std::nullopt;
                // }
            }
            ...
            const auto [fst, snd] = _control.P6dof_Control(_step, {rollC,acc_cmd_v_y,0}, _state, _totalMass, _p_body, _imu_info, targetDis(), 0, _s);
            _rudder               = fst;
            _m_body               = snd;

...
*/
    ...
    void COrbitKit::SatOrbitPredict(double jd, SATELLITE *pSat) {
        ...
        double inc = pSat->oe.inc;
        double omega = pSat->oe.raan;
        double w = pSat->oe.arg_perigee;
        double M0 = pSat->oe.mean_M0;
...
*/
    void
    COrbitKit::MapToOrbitCoord(double *x, double *y, double *z, double alphas, double deltas, double omig, double i,
                               double u) {
        deltas *= PI / 180.0;
        alphas *= PI / 180.0;
        omig *= PI / 180.0;
    ...
    void COrbitKit::AzimuthElevation(double *Azimuth, double *Elevation, double *Range,
        ...
        SinSatLat = sin(SatLat);
        SinSatLong = sin(SatLong);
        CosSatLat = cos(SatLat);
        CosSatLong = cos(SatLong);

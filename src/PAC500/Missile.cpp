...
// region Constructor
    ...
    Missile::Missile() {
        _fileSaver = std::make_shared<FileSaver>("./Results/PAC500/");
        _maxLoad   = 30;
        _s         = 0.223*2;
        _l         = 6.55;
        _b         = 6.55;
        ...
        _kinematics._dynamics._aerodynamics.computeAeroCoefficientsCB =
                [this](const double alpha, const double beta, const double dx, const double dy, const double dz, const double Ma, double &CD, double &CL, double &CZ, double &Cl,
                    ...
                    // 3. 气动力系数
                    CD = -0.7*(CA0 + k * (alpha * alpha + beta * beta));
                    CL = CN * alpha;
                    CZ = -CN * beta;
                    // 4. 气动力矩系数
...
// region Public Methods
    ...
    void Missile::launch(const double theta_f_d, const double psi_f_d) {
        ...
        _state.qbn                     = ModelDevelop::Utils::CoordinateHelper::euler231ToQuaternion(psi_f_d, theta_f_d+30, 0);

...
// region Constructor
    ...
    AircraftModel::AircraftModel() {
        _fileSaver = std::make_shared<FileSaver>("./Results/Aircraft/");
        _maxLoad   = 30;
        _s         = 45.000;
        _l         = 10;
        _b         = 10;
        _mass      = 11267.958;
        _inertia << 14205.197, 0.0, 0.0, 0.0, 118062.182, 0.0, 0.0, 0.0, 118062.182;
    ...
    AircraftModel::~AircraftModel() = default;

// endregion

// region Public Methods
    void AircraftModel::init(const double step, const Eigen::Vector3d &lla) {
        _step         = step;
    ...
    void AircraftModel::take_off(const double theta_f_d, const double psi_f_d) {
        ...
        _flightMode = 0;
        _maneuvering.setManeuvering(ManeuveringModeType::LevelFlight, "", _state);
        _launchFlag = true;
    }

    void AircraftModel::setTargetEcf(const Eigen::Vector3d &targetPosEcf, const Eigen::Vector3d &targetVelEcf, const bool clearQueue) {
        if (clearQueue) {
    ...
    void AircraftModel::setTargetLLA(const Eigen::Vector3d &targetPosLLa, const Eigen::Vector3d &targetVelEcf, const bool clearQueue) {
        if (clearQueue) {
            distance_deque.clear();
        }
    ...
    double AircraftModel::update() {
        ...
        if (_flightMode == 0) {
            ...
            const auto [fst, snd] =_maneuvering.update(_step, _state, _totalMass, _p_body, _imu_info, _s);
            _rudder               = fst;
            _m_body               = snd;
...
// region Private Methods
    Eigen::Vector3d AircraftModel::rk4(const Eigen::Vector3d &_rudder, const Eigen::Vector3d &P_body, const Eigen::Vector3d &M_body) {
        auto yn = _state;
    ...
    Derivative AircraftModel::derivative() const {
    ...
    void AircraftModel::setManeuvering(ManeuveringModeType _type, std::string para){
        _maneuvering.setManeuvering(_type, para, _state);
    }

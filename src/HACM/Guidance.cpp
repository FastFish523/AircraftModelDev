...
// endregion
// region Self
#include "HACM/Guidance.h"

#include <iostream>

#include "CoordinateHelper.h"
#include "HACM/Seeker.h"
// endregion
// endregion
...
// region Static Attributes Init
    ...
    GCInfo Guidance::getGCInfo(const double flyTime, const double P, const double Mass, const Eigen::Vector3d &targetPosEcf, const Eigen::Vector3d &targetVelEcf,
                               const State &state, const double maxLoad, const double desired_h) {
        const Eigen::Vector3d selfPositionEcf = state.posEcf;
        ...
        const Eigen::Vector3d selfVel_nue     = ModelDevelop::Utils::CoordinateHelper::ecefToNueVelocity(state.velEcf, lla.x(), lla.y());
        const auto theta           = ModelDevelop::Utils::CoordinateHelper::getTheta(selfVel_nue);
        Eigen::Vector3d acc_cmd_v  = {0, 0, 0};
        LosInfo losInfo            = {};
        if (flyTime < 0.6) // 策略 无控建立速度
        {
            acc_cmd_v.y() = 0;
            acc_cmd_v.z() = 0;
        } else if (target_dis > 50000) //策略 纵向巡航飞行，侧向朝向目标
        {
            // 制导指令
            ...
            acc_cmd_v.y()   = 9.8 * cos(theta) + 0.05 * (desired_h - lla.z()) - 2 * 4 * 0.05 * Vy;
        } else //末制导
        {
            // 制导指令
    ...
    std::pair<double,double> Guidance::calculateL1Guidance(const double maxLoad,const Eigen::Vector3d &currentPosEcf, const Eigen::Vector3d &currentVelEcf,
        ...
        // 转换到NUE坐标系（以起点为原点）
        Eigen::Vector3d pos_nue = Utils::CoordinateHelper::ecefToNuePosition(currentPosEcf,wp_start_lla.x(),wp_start_lla.y())-Utils::CoordinateHelper::ecefToNuePosition(wp_start_ecf,wp_start_lla.x(),wp_start_lla.y());

        Eigen::Vector3d wp_start_nue(0, 0, 0); // 起点是原点

        Eigen::Vector3d wp_end_nue = Utils::CoordinateHelper::ecefToNuePosition(wp_end_ecf,wp_start_lla.x(),wp_start_lla.y())-Utils::CoordinateHelper::ecefToNuePosition(wp_start_ecf,wp_start_lla.x(),wp_start_lla.y());
        ...
        // L1应该大于最小转弯半径
        double L1_distance = 1.5 * R_min;  // 经验系数
...
// region Private Methods
    LosInfo Guidance::getLOSInfo(const Eigen::Vector3d &targetPosEcf, const Eigen::Vector3d &targetVelEcf, const State &state) {
        ...
        const Eigen::Vector3d target_position_nue = ModelDevelop::Utils::CoordinateHelper::ecefToNuePosition(targetPosEcf, lla.x(), lla.y());
        const Eigen::Vector3d position_nue        = ModelDevelop::Utils::CoordinateHelper::ecefToNuePosition(state.posEcf, lla.x(), lla.y());
        const Eigen::Vector3d target_velocity_nue = ModelDevelop::Utils::CoordinateHelper::ecefToNueVelocity(targetVelEcf, lla.x(), lla.y());
        const Eigen::Vector3d velocity_nue        = ModelDevelop::Utils::CoordinateHelper::ecefToNueVelocity(state.velEcf, lla.x(), lla.y());


        const Eigen::Vector3d rel_pos = target_position_nue - position_nue;
        const Eigen::Vector3d rel_vel = target_velocity_nue - velocity_nue;
        const Eigen::Vector3d rel_w   = rel_pos.cross(rel_vel) / rel_pos.squaredNorm();
    ...
    Eigen::Vector3d Guidance::guidance_pn(const double theta, const double sigma_az_dot, const double sigma_elv_dot, const double dis_dot) {
        ...
        Eigen::Vector3d acc_cmd_v         = Eigen::Vector3d(0, ny_tc, nz_tc);
        return acc_cmd_v;
    }

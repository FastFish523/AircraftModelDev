...
// region Constructor
    ...
    Missile::Missile() {
        _fileSaver = std::make_shared<FileSaver>("./Results/R11/");
        _maxLoad   = 30;
        _s         = 0.223;
        _l         = 6.55;
        _b         = 6.55;
...
// region Public Methods
    ...
    void Missile::launch(const double theta_f_d, const double psi_f_d) {
        ...
        if(!_targetPosEcf.has_value()) {
            std::cout<<"you must set a target!"<<std::endl;
            return;
        }
        auto r_m = _state.posEcf;
        auto lla_m = Utils::CoordinateHelper::ecefToLla(r_m);
        const auto r_t = _targetPosEcf.value();
        double t_pip   = 8 * 60;
        double vd[3], Range;
        _guidance.Lambert_Resolve_Dv1(r_m, r_t, t_pip, vd, Range);
        auto velEcf = Eigen::Vector3d(vd[0],vd[1],vd[2]);
        double t=0;
        while(t<=13) {
            auto g_ecf = Utils::CoordinateHelper::calculateGravity(r_m);
            velEcf = velEcf + g_ecf*0.01;
            r_m = r_m + velEcf*0.01;
            lla_m = Utils::CoordinateHelper::ecefToLla(r_m);
            t+=0.01;
        }
        _cutOffPointVelEcf   = velEcf;
        _cutOffPointPosEcf = r_m;

        const auto temp_VelNue = Utils::CoordinateHelper::ecefToNueVelocity(_cutOffPointVelEcf,lla_m.x(),lla_m.y());
        const auto temp_theta  = Utils::CoordinateHelper::getTheta(temp_VelNue)*57.3;
        const auto temp_psi    = Utils::CoordinateHelper::getPsi(temp_VelNue)*57.3;
        std::cout<<"t:"<<t_pip<<"s range:"<<Range<<"km, theta:"<<temp_theta<<"degree, psi:"<<temp_psi<<"degree "<<"vel:"<<temp_VelNue.norm()<<std::endl;
        _state.qbn                     = Utils::CoordinateHelper::euler231ToQuaternion(temp_psi, temp_theta, 0);

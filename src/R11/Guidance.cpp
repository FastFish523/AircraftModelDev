...
// endregion
// region Self
#include "R11/Guidance.h"
#include "R11/Seeker.h"
...
// region Static Attributes Init
    ...
    GCInfo Guidance::getGCInfo(const double flyTime, const double P, const double Mass, const Eigen::Vector3d &cutPosEcf, const Eigen::Vector3d &cutVelEcf,const Eigen::Vector3d &targetPosEcf, const Eigen::Vector3d &targetVelEcf,
        ...
        LosInfo losInfo            = {};
        if(_boostPhase) {
            const Eigen::Vector3d dv = state.velEcf-cutVelEcf;
            const Eigen::Vector3d dr = state.posEcf-cutPosEcf;
            const Eigen::Vector3d accEcf = -0.0*dr - 1*dv;
            const Eigen::Vector3d acc_cmd_nue = Utils::CoordinateHelper::ecefToNueAcceleration(accEcf,lla.x(),lla.y());
            acc_cmd_v = Utils::CoordinateHelper::nueToVelocity(acc_cmd_nue,selfVel_nue);
            if(flyTime>=13) {
                _boostPhase=false;
            }
        }
        else {
            if (target_dis > 300*1000) //策略 比例中制导
            {
                // 制导指令
                const Eigen::Vector3d omega_earth(0, 0, Utils::Constants::EARTH_OMEGA);
                const Eigen::Vector3d coriolis_accel    = -2.0 * omega_earth.cross(state.velEcf);
                const Eigen::Vector3d centrifugal_accel = -omega_earth.cross(omega_earth.cross(state.posEcf));
                const auto accEcf = -(coriolis_accel+centrifugal_accel);
                auto acc_cmd_nue = Utils::CoordinateHelper::ecefToNueAcceleration(accEcf,lla.x(),lla.y());
                acc_cmd_v = Utils::CoordinateHelper::nueToVelocity(acc_cmd_nue,selfVel_nue);
            }
            else //末制导
            {
    ...
    void Guidance::Lambert_Resolve_Dv1(const Eigen::Vector3d &r_m, const Eigen::Vector3d &r_pip, double &T_pip, double vd_m[3], double &Range) const {
        ...
        if (Range < 9000) {
            ...
            //迭代求解
            gamma0 = (gamma_min*0.5 + gamma_max*0.5);
